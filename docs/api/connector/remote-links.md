...
极限承载接口的输入读取绑定库存槽位；抽回内容直接进入返回库存，不受配置标记和相邻面主动拉取设置控制。链接顺序、模式、槽位和传输游标随方块存档保存。暂时无法进入返回栏的已抽取内容也会保存，后续继续尝试返回。
...
连接器物品、剪贴板、滚轮包和渲染器内部类改用 `RemoteLink` / `Connector` 命名，它们不属于稳定 API。数据组件流编码新增接口槽位和全选状态，客户端与服务端必须使用匹配版本；当前网络协议版本为 `26.9.13`，后续继续采用年、月、日格式。
