...
public final class CancelledCraftingAccountingGameTest {
    ...
    public static void dynamicOutputCancellationIsExactAtomicAndPreservesReceivedAliases(GameTestHelper helper) {
        ...
        ledger.register(ObjectList.of(registration(key, 5L, Route.INVENTORY, first),
                registration(key, 7L, Route.INVENTORY, second)));
        ledger.consume(match(key, 2L, Route.INVENTORY, first), 2L);
        ...
        assertRejectedAtomically(helper, () -> ledger.prepareWithdrawal(ObjectList.of(
                registration(key, 1L, Route.INVENTORY, first),
                registration(key, 1L, Route.FINAL_OUTPUT, second))), () -> ledger.writeToTag(registries));
        assertRejectedAtomically(helper, () -> ledger.prepareWithdrawal(ObjectList.of(
                registration(key, 2L, Route.INVENTORY, first),
                registration(key, 2L, Route.INVENTORY, first))), () -> ledger.writeToTag(registries));
        assertRejectedAtomically(helper, () -> ledger.prepareWithdrawal(ObjectList.of(
                registration(key, 1L, Route.INVENTORY, Data_Energistics.id("unknown_source")))), () -> ledger.writeToTag(registries));

        CompoundTag beforePrepared = ledger.writeToTag(registries);
        Runnable withdrawal = ledger.prepareWithdrawal(ObjectList.of(registration(key, 1L, Route.INVENTORY, first),
                registration(key, 2L, Route.INVENTORY, first)));
        ...
        expected.register(ObjectList.of(registration(key, 7L, Route.INVENTORY, second)));
        ...
        restored.prepareWithdrawal(ObjectList.of(registration(key, 7L, Route.INVENTORY, second))).run();
    ...
    private static Registration registration(AEItemKey key, long amount, Route route, ResourceLocation source) {
        return new Registration(key, BigInteger.valueOf(amount), route, source, ItemMatchingRule.ID, key);
    }

    private static Match match(AEItemKey key, long amount, Route route, ResourceLocation source) {
        return new Match(key, amount, route, source, ItemMatchingRule.ID, key);
    }
