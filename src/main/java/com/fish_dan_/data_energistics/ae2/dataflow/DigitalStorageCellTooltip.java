...
/**
 * Renders capacity, filter state, upgrades, and stored contents for multi-resource digital storage cells.
 */
public final class DigitalStorageCellTooltip {

    private DigitalStorageCellTooltip() {}

    public static void addCellInformation(ItemStack stack, List<Component> lines) {
        DigitalStorageCellInventory inventory = DigitalStorageCellHandler.INSTANCE.getCellInventory(stack, null);
        if (inventory == null) {
            return;
        }
