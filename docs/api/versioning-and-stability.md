...
本文定义 Data Energistics 当前文档所说的“外部 API”范围。它不是对仓库中所有 `public` 类的兼容保证。

## 受支持的外部表面

稳定表面仅包括 `com.fish_dan_.data_energistics.api.**`。新增集成和从旧版本迁移的集成都必须使用 `api/**`。
...
即使这些类型当前是 `public`，也可能在内部整理中直接移动、改名或收窄。外部模组不得：
...
`api/**` 是 3.0.x 起的目标兼容表面。需要外部调用者迁移的 API 清理应通过文档和弃用信息明确说明，而不是依赖内部包恰好未变化。

以下 Universal Terminal 类型仅在 3.0.x 支持周期内作为过渡 API 保留，并已标记 `@Deprecated(forRemoval = true)`：
...
它们已在 3.1.0 移除，不再属于发布产物或外部兼容表面。迁移目标和方法映射见[旧 API 迁移](terminal/legacy-migration.md)。
